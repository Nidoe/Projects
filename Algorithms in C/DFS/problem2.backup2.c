...
int main(int argc, char *argv[])
{
		...
		*/
		...
		for(i = 1; i<= vertices; i++){
			...
			printf("Vertex %d has a root of %d\n",i, temp->root);
		}
		*/
		...
		for(i=1; i<= vertices; i++){
			...
			struct list *current;
			current = (struct list*)malloc(sizeof(struct list*));
			current = (struct list*)componentList[i];
		...
		for(i=1; i<=numRoots; i++){
			...
			for(j=1; j<=numRoots; j++){
				...
				if(incidenceMatrix[i][j] == 0 && i != j){
					printf("\ni : %d with rootptr %d, j : %d with rootptr %d\n",i, rootptr[i],j, rootptr[j]);
					runtime = runtime + 1;
					struct list *current;
					int root1;
					int root2;
							...
							while(current->next != NULL){	
								...
								if(adjList->data != NULL){
									while(adjList->next != NULL){	
										...
										printf("Vertex %d with edge to: %d", n, adjList->data);
