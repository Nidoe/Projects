...
void DFS(int index,int* vList[]){
	...
	temp = (struct Vertex*)vList[index];
	...
	if(current->data != NULL){
		while(current->next != NULL){	
			temp = (struct Vertex*)vList[current->data];	
			if(temp->visited == 0){
...
int main(int argc, char *argv[])
{
		...
		for(i = 1; i<= vertices; i++){
			...
			temp = (struct Vertex*)vertexList[i];
		...
		for(i = 1; i<= vertices; i++){
			...
			temp = (struct Vertex*)vertexList[index];
							...
							while(current->next != NULL){	
								...
								if(adjList->data != NULL){
									while(adjList->next != NULL){	
										...
										v = (struct Vertex*)vertexList[adjList->data];	
										root2 = v->root;
